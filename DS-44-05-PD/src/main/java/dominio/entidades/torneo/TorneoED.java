 ...
 */
package dominio.entidades.torneo;
...
import dominio.torneo.Ronda;
import dominio.torneo.abbmodel.Abb;
import dominio.torneo.abbmodel.Nodo;
import dominio.torneo.abbmodel.NodoJugador;
import dominio.torneo.abbmodel.NodoPartido;
import dominio.torneo.abbmodel.iterator.TreeIterator;
...
public  class TorneoED extends Abb implements TorneoValue{
	...
	@Override
	public List<PartidoValue> getPartidos() {
		return null;
	}
            ...
            try {
                ...
                for(T nodo : tabla.get(1)){
                    ...
                    if(nodo instanceof NodoPartido){
                        PartidoValue partido = ((NodoPartido) nodo).getPartido();
        ...
        try {
            if((this.getAltura()-1 > this.getRondaEnJuego())){
                ...
                for(NodoPartido nodo: nodos){
                    PartidoValue partido = nodo.getPartido();
	...
	private <T extends JugadorValue> void generarTabla(int altura, int aux, Abb arbol, Deque<T> jugadores){
            if (altura == 2) {
		arbol.addNodo(new NodoPartido(aux, new PartidoValue("#MTCH_"+(altura+aux))));
        ...
        if(nodo instanceof NodoPartido){
            String tab = tabs;
            PartidoValue p = ((NodoPartido) nodo).getPartido();
